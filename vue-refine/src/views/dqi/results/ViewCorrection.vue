<template>
  <div class="mt-10">
    <v-card elevation="2">
      <v-toolbar color="blue-grey darken-3" dark height="40" flat
        >Column Selection
      </v-toolbar>
          ...
          <template v-if="dataIndexItem === '결측률'">
            <v-col cols="2" class="d-flex justify-center pt-0">
              <v-radio-group v-model="selected">
                ...
                <v-radio label="삭제" color="blue-grey darken-4" value="delete">
                </v-radio>
              </v-radio-group>
            </v-col>
          </template>
...
export default {
  components: {
    CorrectionChartDoughnut: () =>
      import("./comp/correction-chart-doughnut.vue")
  },
  data() {
    return {
      selected: "imputation",
      dataIndexList: [
        { item: "결측률", disable: false },
        { item: "패턴 불일치", disable: false },
        { item: "타입 불일치", disable: false }
      ],
      columnNameList: [],
      columnName: "",
      dataIndexItem: "결측률",
          ...
          api_dataDqi(ruleModeParam).then((response) => {
            if (response.result !== "SUCCESS") {
              ...
              vm.EventBus.$emit("modalAlert", {
                title: "경고",
                text:
                  "데이터 조회를 하지 못했습니다. 잠시후 다시 시도해 주세요",
                okTitle: "확인"
              });
            } else {
