...
QEDialogUI.prototype._getStatisticData = function(selectedHeaders) {
...
QEDialogUI.prototype._createDialog = function(selectedHeaders) {
	_QEDialogUI = this;
	...
	$(window).resize((target) => {
		if (_QEDialogUI != null) {
			_QEDialogUI._createChartTemplate();
			_QEDialogUI._createChart();
		}
	})
...
QEDialogUI.prototype._createChartTemplate = function() {
	...
	for (var i = 0; i < _len; i++) {
		...
		if (ci == undefined) {
			...
			trTemplate2 += '<td>';
			trTemplate2 += '<div class="qe-chart" id="'+CHART_DIV_ID+'_'+i+'">';
			trTemplate2 += '</div>';
			trTemplate2 += '</td>';
		}
...
QEDialogUI.prototype._createChart = function() {
	var chartDivs = $('div[id^="qe_chart"]');
	var _self = this;
	
	var margin = ({top: 5, right: 5, bottom: 5, left: 5})
	
	const _p = $(chartDivs[0]);
	const width = _p.width();
	const height = _p.height();
	
	var chartInfo = null;

	// get data
	const chartData = this._getStatisticData();
	
	chartDivs.each((i, _div)=>{
		chartInfo = _self.chartInfos[i];
		
		_self.createChartByType(chartInfo.type, {
			_tempData : chartData[i],
			pId: _div.id, 
			chartInfo: chartInfo, 
			width: width, 
			height: height, 
...
function createChart_bar(params) {
	...
	const chartInfo = params.chartInfo;
	const tempData = params._tempData;
